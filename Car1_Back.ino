...
void loop() {
  ...
  if(FinishLine>=4 && x==0) {
    ...
    if(CurrentDistance <60 ) DistanceError = CurrentDistance/2 + (CurrentDistance-PreviousDistance)/10;
    else DistanceError = 30;
    
    if(DistanceError>=30) DistanceError=30;
    if(CurrentDistance < 20) {
      DistanceError=0;
    }
    Brushless = 1225+DistanceError;
    myesc.writeMicroseconds(Brushless);
...
int Distance() {
  ...
  //digitalWrite(MiddleTrig, LOW);
  //digitalWrite(RightTrig, LOW);
  delayMicroseconds(2);
  
  digitalWrite(LeftTrig, HIGH);
  //digitalWrite(MiddleTrig, HIGH);
  ...
  duration1 = pulseIn(LeftEcho, HIGH);
  //duration2 = pulseIn(MiddleEcho, HIGH);
  //duration3 = pulseIn(RightEcho, HIGH);
  ...
  //shortest1 = min(dist1,dist2);
  //shortest1 = min(shortest1,dist3);
